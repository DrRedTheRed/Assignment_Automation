...
def solve(pose):
    ...
    # 如果是单个 6 维向量，转换为 (1,6) 形状
    if pose.shape == (6,):
        pose = pose.reshape(1, 6)

    d_x, d_y, d_z, alpha, beta, gamma = pose.T  # 转置以便批量计算
    ...
    ans = np.vstack([theta_1, theta_2, theta_3, theta_4, theta_5, theta_6])

    # 过滤 NaN 解
    cols_with_nan = np.isnan(ans).any(axis=0)
    ans = ans[:, ~cols_with_nan]

    return ans if ans.shape[1] > 0 else np.array([])
...
if __name__ == "__main__":
    ...
    solutions = solve(np.array([-0.2, 0.35, 0.23, 3.14, -0.06, -3.14]))
